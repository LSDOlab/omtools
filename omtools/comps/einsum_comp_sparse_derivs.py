...
from miscellaneous_functions.process_options import name_types, get_names_list, shape_types, get_shapes_list
...
class EinsumComp(ExplicitComponent):
    ...
    def setup(self):
        ...
        for in_name_index, in_name in enumerate(in_names):
            ...
            flattened_input_index = np.array([0,])
            flattened_output_index = np.array([0,])

            out_shape = self.out_shape
            new_output_tensor_rep = operation_aslist[-1] + self.unused_chars[:len(operation_aslist[in_name_index])]
            partial_rank = len(new_output_tensor_rep)
            
            full_partial_indices_list = [np.array([0,])] * partial_rank
            ...
            for loc in locations:
                ...
                if self.surviving_axes_map[loc]:
                    ...
                    remainder_output_ind = np.unravel_index(remainder_flat_indices, remainder_output_shape)
                    
                    i = 0
                    # print(*(k.size for k in partial_indices_list[3:]))
                    for idx, indices in enumerate(partial_indices_list):
                        if isinstance(indices, np.ndarray):
                            partial_indices_list[idx] = np.tile(indices, remainder_output_size)
                        elif isinstance(indices, int):
                            partial_indices_list[idx] = np.repeat(remainder_output_ind[i], size)
                            i += 1
                            # print(idx)
                        else:
                            raise Exception()
                    ...
                    for k in range(partial_rank):
                        full_partial_indices_list[k] = np.append(full_partial_indices_list[k], partial_indices_list[k])
            
            if sparse_partial:
                sparse_partial = False   
                
                partial_size = full_partial_indices_list[0].size
                full_partial_indices_array = full_partial_indices_list[0].reshape((partial_size, 1))

                for k in range(1, partial_rank):    
                    full_partial_indices_array = np.append(full_partial_indices_array, full_partial_indices_list[k].reshape((partial_size, 1)), axis = 1)
                full_partial_indices_array = np.unique(full_partial_indices_array, axis=0)
                full_partial_indices_list = [full_partial_indices_array[:, x] for x in range(partial_rank)]

                flattened_input_index = np.ravel_multi_index(full_partial_indices_list[-rank:], shape)
                flattened_output_index = np.ravel_multi_index(full_partial_indices_list[:-rank], out_shape)

                self.declare_partials(out_name, in_name, rows = flattened_output_index, cols = flattened_input_index)
                # self.partial_indices_list.append(partial_indices_list)
                self.partial_indices_list.append(full_partial_indices_list)
                self.flattened_indices_list.append((flattened_input_index,flattened_output_index))
    ...
    def compute_partials(self, inputs, partials):
        ...
        for in_name_index, in_name in enumerate(in_names):
            ...
            partial = np.einsum(new_operation, *(inputs[in_name] for in_name in in_names[:in_name_index]), self.I[in_name_index], *(inputs[in_name] for in_name in in_names[in_name_index + 1 :]))

            for i in locations[1:]:
                new_operation_aslist = deepcopy(operation_aslist)
                new_in_name_tensor_rep = operation_aslist[i] + unused_chars[: rank]
                ...
                partial += np.einsum(new_operation, *(inputs[in_name] for in_name in in_names[:i]), self.I[len(completed_in_names)-1], *(inputs[in_name] for in_name in in_names[i + 1 :]))

            # Partials are dense if surviving_axes_map is empty
            if self.surviving_axes_map[in_name_index]:
                partial_indices_list = self.partial_indices_list[len(completed_in_names) - 1]
