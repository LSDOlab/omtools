...
class MatMatComp(ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input(in_names[0], shape=in_shapes[0])
        self.add_input(in_names[1], shape=in_shapes[1])
        ...
        c0 = np.tile(np.arange(input1_size).reshape(in_shapes[0]), in_shapes[1][1]).flatten()

        self.declare_partials(out_name, in_names[0], rows=r0, cols=c0)

        r1 = np.repeat(np.arange(output_size), in_shapes[0][1])
        c1 = np.tile(np.transpose(np.arange(input2_size).reshape(in_shapes[1])).flatten(), in_shapes[0][0])
    ...
    def compute_partials(self, inputs, partials):
        ...
        partials[out_name, in_names[0]] = np.tile(np.transpose(inputs[in_names[1]]).flatten(), in_shapes[0][0]) 
        
        partials[out_name, in_names[1]] = np.tile(inputs[in_names[0]], in_shapes[1][1]).flatten()
...
if __name__ == "__main__":
    ...
    prob.model.add_subsystem(
        'matmat',
        MatMatComp(in_names=['mat1','mat2'], out_name='y', in_shapes=[shape1, shape2], 
        ),
        promotes=['*']
    )
