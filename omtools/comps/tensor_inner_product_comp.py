...
class TensorInnerProductComp(ExplicitComponent):
    ...
    def setup(self):
        ...
        self.new_axes_locations = list(np.arange(num_rem_axes0)) + list(np.arange(num_partial_axes0, total_num_partial_axes0)) + list(np.arange(num_rem_axes0, num_partial_axes0))
    ...
    def compute(self, inputs, outputs):
        ...
        outputs[out_name] = np.tensordot(inputs[in_names[0]], inputs[in_names[1]], axes=axes)
    ...
    def compute_partials(self, inputs, partials):
        ...
        partials[out_name, in_names[0]] = np.transpose(temp_partials, self.new_axes_locations)
        partials[out_name, in_names[1]] = np.tensordot(inputs[in_names[0]], self.I1, axes=axes)
