...
class EinsumComp(ExplicitComponent):
    ...
    def setup(self):
        ...
        rows = np.einsum(einsum_string, out_indices, np.ones(ones_shape,
                                                             int)).flatten()
        cols = in_indices.flatten()

        # We're going to have some kind of loop over each in_name here
        # At each iteration, we're going to declare a new partial derivative
        # TODO: make list of in_names
        # val will be a product of all tensors that are not out_name
