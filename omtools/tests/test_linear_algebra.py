...
def test_dot():
    ...
    np.testing.assert_almost_equal(example.prob['TensorVecDot'],
                                   desired_output)

    partials_error = example.prob.check_partials(includes=['comp_TenVecDot'],
                                                 out_stream=None,
                                                 compact_print=True)
    assert_check_partials(partials_error, atol=1.e-6, rtol=1.e-6)
    ...
    partials_error = example.prob.check_partials(includes=['comp_TenTenDot'],
                                                 out_stream=None,
                                                 compact_print=True)
